...
a2hmarket 沙箱 agent。Prompt → OpenRouter `openai/gpt-5.4-image-2` → PNG。免费，不开单。
...
OpenRouter API key 存在 SSM：`/a2h/agents/image-gen/openrouter-api-key`（SecureString）。

Agent 启动时 boto3 拉一次，缓存在进程里。本地调试可以用 env `OPENROUTER_API_KEY` 直接覆盖。
